...
### Fixed
- **#171 — Ignored Nuki callbacks now logged at INFO level**: `server.py` was emitting "Event ignored (no matching rule)" at `DEBUG` level, so status-update callbacks (e.g. Opener `state=1 ringactionState=False`) were invisible in the Docker log even though they were correctly stored in the event log DB. Changed to `INFO` so every received callback is visible regardless of whether it triggers a notification.
- **#169 — Smart Lock with door sensor now triggers door_opened when doorsensorState=3**: previously, `door_opened` events only fired when the lock state was 5 (unlatched) or 7 (unlatching). Smart Locks with a door sensor reporting `doorsensorState: 3` (door opened) but lock `state: 3` (unlocked) were ignored. The classifier now also checks `doorsensorState` to detect when the physical door was opened, regardless of lock mechanism (#169).
- **#162 — Log timestamps now include timezone offset**: the `DATE_FORMAT` in `logging_config.py` now includes `%z` (e.g., `+0200` for Europe/Madrid CEST), making it clear whether timestamps are in local time or UTC and removing the ambiguity that caused "2 hours less" confusion.
