...
- **#149**: "Send support bundle to GitHub issue" returned a bare **HTTP 400** with no clue why. The two 400 paths in `POST /api/support/github-issue` (missing token, and `SupportBundleError`) **logged nothing** server-side, and `support_bundle.build_and_send` collapsed GitHub HTTP failures into terse strings (`"GitHub API error (HTTP 404)"`) that **discarded GitHub's response body**. Now both 400 paths log a WARNING with the reason; a new `_github_error_detail()` extracts GitHub's `message`/`errors` (or the raw body) and includes it in the error, plus a dedicated 404 message naming the target `repo` (the most common cause: wrong `github.repo` or a token lacking access to it). The UI already surfaces `error`, so the actual cause is now visible both in the UI and the logs. Added regression tests.
